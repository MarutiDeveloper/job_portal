...
function clearValidationErrors() {
    // List of form fields to clear validation for
    const fields = ["title", "category", "jobType", "vacancy", "location", "description", "company_name"];
...
function handleValidationErrors(errors) {
    for (const [field, messages] of Object.entries(errors)) {
        if (messages.length > 0) {
            $("#" + field).addClass('is-invalid').siblings('p').addClass('invalid-feedback').html(messages[0]);
        } else {
            $("#" + field).removeClass('is-invalid').siblings('p').removeClass('invalid-feedback').html('');
        }
    }
